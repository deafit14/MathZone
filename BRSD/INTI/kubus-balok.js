...
// === FUNGSI BARU: UPDATE LABEL BALOK ===
function updateLabelBalok() {
  ...
  const lblP = document.getElementById("lbl_p");
  const lblL = document.getElementById("lbl_l");
  const lblT = document.getElementById("lbl_t");

  // Perbarui teks jika elemennya ada
  if (lblP) lblP.innerText = p;
  if (lblL) lblL.innerText = l;
...
// === LOGIKA HIGHLIGHT OTOMATIS SAAT INPUT DIKLIK ===

function setupHighlightEffects(prefixInput, idBangunRuang) {
  // Ambil elemen input
  const inputSisi = document.getElementById(`${prefixInput}_sisi`);
  const inputRusuk = document.getElementById(`${prefixInput}_rusuk`);
  const inputTitik = document.getElementById(`${prefixInput}_titik`);

  // Ambil elemen 3D bangun ruang
  const bentuk3D = document.getElementById(idBangunRuang);

  if (!inputSisi || !bentuk3D) return; // Mencegah error jika elemen tidak ditemukan

  // Event untuk Sisi
  inputSisi.addEventListener("focus", () =>
    bentuk3D.classList.add("highlight-sisi"),
  );
  inputSisi.addEventListener("blur", () =>
    bentuk3D.classList.remove("highlight-sisi"),
  );

  // Event untuk Rusuk
  inputRusuk.addEventListener("focus", () =>
    bentuk3D.classList.add("highlight-rusuk"),
  );
  inputRusuk.addEventListener("blur", () =>
    bentuk3D.classList.remove("highlight-rusuk"),
  );

  // Event untuk Titik Sudut
  inputTitik.addEventListener("focus", () =>
    bentuk3D.classList.add("highlight-titik"),
  );
  inputTitik.addEventListener("blur", () =>
    bentuk3D.classList.remove("highlight-titik"),
  );
}

// Jalankan fungsi setelah seluruh halaman selesai dimuat
document.addEventListener("DOMContentLoaded", () => {
  // Pasang efek untuk bagian Identifikasi Kubus (prefix input: 'k', id bangun: 'cube_ident')
  setupHighlightEffects("k", "cube_ident");

  // Pasang efek untuk bagian Identifikasi Balok (prefix input: 'b', id bangun: 'balok_ident')
  setupHighlightEffects("b", "balok_ident");
});
